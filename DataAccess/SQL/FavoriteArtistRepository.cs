...
namespace DataAccess.Implementation.SQL
{
	public class FavoriteArtistRepository : IFavoriteArtistRepository
	{
		...
		public async Task<IEnumerable<Entities.Artist>> GetFavoriteArtists(int userId)
		{
		...
		public async Task<IEnumerable<Entities.Artist>> GetFavoriteArtistsByName(string artistName, int userId)
		{
			IList<Entities.Artist> favoriteArtists = new List<Entities.Artist>();
		...
		public int SaveFavoriteArtist(Entities.Artist artist, int userId)
		{
			...
			string imageUrl;
			var image = artist.Image.FirstOrDefault(i => string.Equals(i.Size, LargeImageSize, System.StringComparison.Ordinal));
			if (image != null)
			{
				imageUrl = image.Text;
			}
